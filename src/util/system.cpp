...
const char * const LLAMACOIN_CONF_FILENAME = "llamacoin.conf";
 ...
 * https://github.com/llamacoin/llamacoin/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //llamacoin-tx using stdin
        std::string val;
...
#else
    const char* pszModule = "llamacoin";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Llamacoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Llamacoin
    // Mac: ~/Library/Application Support/Llamacoin
    // Unix: ~/.llamacoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Llamacoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Llamacoin";
#else
    // Unix
    return pathRet / ".llamacoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", LLAMACOIN_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Llamacoin Core copyright is not removed by accident
    if (copyright_devs.find("Llamacoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Llamacoin Core developers";
    }
