...
static void SetupLlamacoinTxArgs()
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " llamacoin-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  llamacoin-tx [options] <hex-tx> [commands]  Update hex-encoded llamacoin transaction\n" +
            "or:     llamacoin-tx [options] -create [commands]   Create hex-encoded llamacoin transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded llamacoin transaction
